...
def do_scrape(s_pos, e_pos):
    ...
    areas = ["bakersfield", "chico", "fresno", "goldcountry", "hanford", "humboldt", "mendocino", "merced", "modesto",
             "monterey", "redding", "reno", "sacramento", "sfbay", "slo", "santabarbara", "santamaria", "siskiyou", "stockton", "susanville", "ventura", "visalia", "yubasutter"]
    ...
    for i in range(s_pos, e_pos):
        ...
        for j in range(0, len(areas)):
            if (areas[j] == "sfbay"):
                ...
                url = 'https://'+areas[j]+'.craigslist.org/cto/'+str(i)+'.html'
                soup = get_request(url)
                if soup == False or soup == None:
                    if soup == False:
                        insert_thread_limit(i)
                    ...
                    make_data(soup, url, i, areas[j], None)
                    break
...
def insert_thread_limit(limit_id):
    try:
        cursor = connect.cursor()
        cursor.execute(
            "INSERT INTO car_trucks_limit(limit_id) VALUES("+str(limit_id)+")")
        cursor.close()
    except Error as e:
        print('error:', e)
...
def get_thread_limit():
    try:
        cursor = connect.cursor()
        cursor.execute(
            "SELECT *FROM car_trucks_limit ORDER BY limit_id LIMIT 1")
        result = cursor.fetchone()
        cursor.execute("TRUNCATE TABLE car_trucks_limit")
        cursor.close()
        return result
    except Error as e:
        print('error:', e)
